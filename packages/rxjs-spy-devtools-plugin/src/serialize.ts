...
function str(key, holder, depthDecr: number, arrayMaxLength: number, replacer: SerializeReplacer, seen = new WeakSet()) {
    ...
    case 'object':
      ...
      if (Object.prototype.toString.apply(value) === '[object Array]') {
        ...
        for (i = 0; i < length; i += 1) {
          partial[i] = str(i, value, depthDecr - 1, arrayMaxLength, replacer, seen) || 'null';
        }
        v = partial.length === 0
          ? '[]'
          : '[' + partial.join(',') + ']';
...
const jsonPrune = function (value, depthDecr, arrayMaxLength, replacer?: SerializeReplacer) {
  depthDecr = depthDecr || DEFAULT_MAX_DEPTH;
  arrayMaxLength = arrayMaxLength || DEFAULT_ARRAY_MAX_LENGTH;
  return str('', { '': value }, depthDecr, arrayMaxLength, replacer);
};

const serialize = (obj: any, replacer?: SerializeReplacer) => {
