...
const handlePostMessageUpdate = reducer(getPostMessage$(), (state: EmissionState, message) => {
  if (isBatch(message)) {
    return {
      emissions: limitEmissions([
        ...state.emissions,
        ...message.data.filter(msg => msg.messageType === MessageTypes.NOTIFICATION).map(({ data }) => {
          let value = {
